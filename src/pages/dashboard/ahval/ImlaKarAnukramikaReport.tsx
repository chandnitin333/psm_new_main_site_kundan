...
/* इमलाकार अनुक्रमणिका — same as old `imla-kar-anukramnika-list`.
   Flat index of इमलाकर properties. Filters via sessionStorage 'imlakarAnukramikaParams'. */

type Row = Record<string, unknown>;
const s = (v: unknown) => (v === null || v === undefined ? '' : String(v));

const ImlaKarAnukramikaReport = () => {
  ...
  useEffect(() => {
    ...
    document.title = 'इमलाकार अनुक्रमणिका';
    let params: { ward?: string; start?: string; end?: string; year?: string } = {};
    try {
      params = JSON.parse(sessionStorage.getItem('imlakarAnukramikaParams') || '{}');
    } catch {
      params = {};
    }
    setWard(params.ward || '');
    (async () => {
      try {
        ...
        console.error('Failed to load imlakar anukramika', e);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const shareParams = (() => { try { return JSON.parse(sessionStorage.getItem('imlakarAnukramikaParams') || '{}'); } catch { return {}; } })();
  const qrUrl = useReportShareUrl({ reportType: 'imlakar-anukramika', sessionKey: 'imlakarAnukramikaParams', params: shareParams, data: records, enabled: !isPublicReportMode() });

  return (
    <div className="ika-report bg-white text-black p-4" style={{ colorScheme: 'light' }}>
      <style>{`
        ...
        @media print {
          ...
          .ika-report th, .ika-report td { font-size: 14px !important; line-height: 1.35 !important; padding: 5px 8px !important; }
          .ika-report thead { display: table-header-group; }
        }`}</style>
        ...
        {!isPublicReportMode() && (
          <button
            onClick={() => {
              ...
              window.open('/view-anukramika-card?variant=imlakar', '_blank');
            }}
      ...
      <div className="mx-auto" style={{ maxWidth: '1000px' }}>
        <div className="text-center">
          <p className="font-bold text-lg">इमलाकार अनुक्रमणिका</p>
        </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              <th className={th}>वार्ड क्रमांक.</th>
              <th className={th}>मालमत्ता क्रमांक.</th>
...
export default ImlaKarAnukramikaReport;
