...
const ShouchalayReport = () => {
  ...
  const [shouchalayRecords, setShouchalayRecords] = useState<ShouchalayRecord[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const wardDataString = sessionStorage.getItem('shouchalayReportWard');
      if (wardDataString) {
        const data = JSON.parse(wardDataString);
        setWardData(data);

        // Generate 20 sample shouchalay records for demo
        const records: ShouchalayRecord[] = [];
        const recordCount = 20;
        const toiletTypes = ['आहे', 'नाही', 'बांधकामाधीन'];
        ...
        for (let i = 1; i <= recordCount; i++) {
          ...
          records.push({
            ...
            shouchalay: toiletTypes[Math.floor(Math.random() * toiletTypes.length)],
            panNo: generatePAN(),
          });
        }
        setShouchalayRecords(records);
      }
