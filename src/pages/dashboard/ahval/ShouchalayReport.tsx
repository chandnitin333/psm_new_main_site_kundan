...
const ShouchalayReport = () => {
  ...
  useEffect(() => {
    ...
    document.title = 'शौचालय यादी';
    const w = sessionStorage.getItem('shouchalayReportWard') || '';
    setWard(w);
    (async () => {
      try {
        ...
        console.error('Failed to load shouchalay ward list', e);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const exportCols: ExportColumn<Row>[] = [
    ...
    { header: 'शौचालय', value: (r) => s(r.ghari_souychalaya), width: 14 },
  ];

  return (
    <div className="shouchalay-report bg-white text-black p-4" style={{ colorScheme: 'light' }}>
      <style>{`
      ...
      <div className="no-print mb-4 flex items-center gap-3">
        ...
        {!isPublicReportMode() && (
          <button
            onClick={() => {
              ...
              window.open('/view-anukramika-card?variant=shouchalay', '_blank');
            }}
        ...
        {!isPublicReportMode() && (
          <ExportButtons
            moduleKey="ahval_shouchalay_list"
            size="md"
            columns={exportCols}
            rows={records}
            filename={`shouchalay-yadi-ward-${ward || 'all'}`}
            title="शौचालय यादी"
      ...
      <div className="mx-auto" style={{ maxWidth: '900px' }}>
        <div className="text-center">
          <p className="font-bold text-lg">शौचालय यादी</p>
        </div>
            ...
            {records.length === 0 ? (
              ...
              records.map((item, i) => (
                <tr key={i}>
                  ...
                  <td className={td}>{s(item.ghari_souychalaya)}</td>
...
export default ShouchalayReport;
