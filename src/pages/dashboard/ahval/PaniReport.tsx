...
interface WardData {
  ...
  waterConnected: number;
}

interface PaniRecord {
  srNo: number;
  milkatNo: string;
  khateDharak: string;
  bhogwatdar: string;
  waterArrangement: string;
  panNo: string;
}
...
const PaniReport = () => {
  ...
  const [paniRecords, setPaniRecords] = useState<PaniRecord[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const wardDataString = sessionStorage.getItem('paniReportWard');
      if (wardDataString) {
        const data = JSON.parse(wardDataString);
        setWardData(data);

        // Generate 20 sample pani records for demo
        const records: PaniRecord[] = [];
        const recordCount = 20;
        const waterTypes = ['नळ जोडणी', 'सार्वजनिक नळ', 'विहीर', 'बोअरवेल', 'इतर'];
        ...
        for (let i = 1; i <= recordCount; i++) {
          ...
          records.push({
            ...
            waterArrangement: waterTypes[Math.floor(Math.random() * waterTypes.length)],
            panNo: generatePAN(),
          });
        }
        setPaniRecords(records);
      }
