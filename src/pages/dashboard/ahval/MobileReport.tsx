...
const MobileReport = () => {
  ...
  const [mobileRecords, setMobileRecords] = useState<MobileRecord[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const wardDataString = sessionStorage.getItem('mobileReportWard');
      if (wardDataString) {
        const data = JSON.parse(wardDataString);
        setWardData(data);

        // Generate 20 sample mobile records for demo
        ...
        for (let i = 1; i <= recordCount; i++) {
          ...
          records.push({
            ...
            mobileNo: `${Math.floor(7000000000 + Math.random() * 3000000000)}`,
            panNo: generatePAN(),
          });
        }
        setMobileRecords(records);
      }
