...
/* एका record मधील मालमत्तेचे वर्णन (land + बांधकाम + मनोरा) खूप rows असल्यास एका पानात बसत नाहीत.
   म्हणून descriptions ला DESC_ROWS_PER_PAGE नुसार अनेक पानांत विभागतो. प्रत्येक पानावर तेच
   header + property details राहतात; कराची रक्कम / फेरफार part प्रत्येक पानावर 0 दिसतो आणि
   खरी रक्कम फक्त शेवटच्या पानावर दिसते (म्हणजे continue होत आहे असे समजते). */
...
const RecordBlock = ({ n, loc, cy, qrUrl, blank = false }: { n: Row; loc: { district: string; taluka: string; gramPanchayat: string }; cy: number; qrUrl?: string; blank?: boolean }) => {
  ...
  const pageCount = blank ? 1 : Math.max(1, Math.ceil(descRows.length / DESC_ROWS_PER_PAGE));

  const fillerRow = (key: string) => (
    <tr key={key} style={{ height: '28px' }}>
  ...
  const pages = Array.from({ length: pageCount }, (_, p) => {
    ...
    const slice = blank ? [] : descRows.slice(p * DESC_ROWS_PER_PAGE, (p + 1) * DESC_ROWS_PER_PAGE);
    // फक्त blank form ला रिकाम्या ओळी; value असलेल्या पानांवर जितके rows तितकेच (pad नको)
    const fillerCount = blank ? DESC_ROWS_PER_PAGE : 0;
    // कराची रक्कम / फेरफार ची values: शेवटच्या पानावरच खरी; बाकी पानांवर 0 (continue होत आहे)
    const cell = (real: unknown) => (last ? fv(real) : '0');

    return (
      <div className="n8m-page mx-auto" style={{ width: `${tableW}px` }} key={`page-${p}`}>
        ...
        {/* Taxation table (2) — या पानाचे description rows (उर्वरित pad करून समान उंची) */}
        <table className="table-fixed border-collapse mb-2" style={{ width: `${tableW}px` }}>
          ...
          <tbody>
            ...
            {slice}
            {Array.from({ length: fillerCount }).map((_, i) => fillerRow(`fill-${p}-${i}`))}
          </tbody>
        </table>
        ...
        <table className="table-fixed border-collapse" style={{ width: `${tableW}px` }}>
          ...
          <tbody>
            ...
            <tr style={blankH}>
              ...
              <td className={`${td} text-left`} colSpan={12}>{last ? sv(n.magahun_ghat_kiva_badal) : ''}</td>
        ...
        <div className="text-right text-sm">पान नंबर : {sv(n.anu_kramank)}{pageCount > 1 ? ` (${p + 1}/${pageCount})` : ''}</div>
      </div>
    );
  });
