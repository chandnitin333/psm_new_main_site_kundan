...
/* सरकारी नमुना ८ (Sarkari Namuna 8) — exact old-app layout.
   ...
   Uses the same full nodni record (main + land/construction/manora + other tax) as नमुना ८. */
...
const Namuna8SarkariPrint = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('Failed to load namuna-8-sarkari data', e);
      }
    })();
  }, []);
  ...
  // first section/data row = land (खुला भूखंड); subsequent rows = construction then manora.
  // land (खुला भूखंड) can have multiple records — render them all. Fall back to one blank row.
  const landRows = land.length ? land : [{} as Row];
  const a = landRows.length + cons.length + manora.length; // rowspan for the left identity / right tax cells

  const td = 'border border-black px-1.5 py-1 text-[11px] align-middle text-center';
  const thc = `${td} font-bold bg-gray-100`;

  // Shared 15-column fixed grid (both sections map onto these 15 columns).
  // table-fixed + colgroup keeps every row's columns aligned; print zoom shrinks to A4 landscape.
  const colW = [55, 70, 80, 75, 90, 75, 75, 85, 75, 90, 70, 80, 80, 75, 90];
  const tableW = colW.reduce((x, y) => x + y, 0);

  return (
    <div className="namuna8s-report bg-white text-black p-4" style={{ colorScheme: 'light' }}>
      <style>{`
...
export default Namuna8SarkariPrint;
