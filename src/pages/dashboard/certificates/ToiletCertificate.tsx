...
/* शौचालय प्रमाणपत्र (Toilet Certificate) — certifies personal toilet & regular use. */
const ToiletCertificate = () => {
  const { toast, ToastContainer } = useToast();
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [f, setF] = useState({
    name: '', relation: 'वडील', parent: '', address: '',
    propertyNo: '', hasToilet: 'होय', regularUse: 'होय', purpose: '',
  ...
  const canAdd = can(certModuleKey('toilet'), 'add');
  const canPrint = can(certModuleKey('toilet'), 'print');
  useEffect(() => {
    ...
    <div className="p-4 sm:p-6 print:p-0">
      ...
      <div className={isView ? '' : 'grid grid-cols-1 gap-6 lg:grid-cols-2'}>
        ...
        <div className="no-print rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">शौचालय प्रमाणपत्र — तपशील भरा</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className={label}>वैयक्तिक शौचालय</label>
              <select name="hasToilet" value={f.hasToilet} onChange={on} className={inp}>
                <option value="होय">होय</option>
                <option value="नाही">नाही</option>
              </select>
            </div>
            <div>
              <label className={label}>नियमित वापर</label>
              <select name="regularUse" value={f.regularUse} onChange={on} className={inp}>
                <option value="होय">होय</option>
                <option value="नाही">नाही</option>
              </select>
            </div>
            <div className="sm:col-span-2">
              ...
              <MarathiInput name="purpose" value={f.purpose} onChange={on} className={inp} placeholder="उदा. योजना लाभ" />
            </div>
            <div>
        ...
        <div className={isView ? '' : 'lg:sticky lg:top-4 lg:self-start'}>
          <CertificateLayout
            title="शौचालय प्रमाणपत्र"
            subtitle="(Toilet Certificate)"
          ...
          >
            <p className="text-justify">
              ...
              <strong>{f.address || '____________'}</strong>, यांचे घरी (मालमत्ता क्र.{' '}
              <strong>{f.propertyNo || '________'}</strong>) वैयक्तिक शौचालय{' '}
              <strong>{f.hasToilet || '________'}</strong> असून त्याचा नियमित वापर{' '}
              <strong>{f.regularUse || '________'}</strong> केला जातो. सदर कुटुंब हागणदारीमुक्त आहे.
            </p>
            <div className="mt-4">
...
export default ToiletCertificate;
