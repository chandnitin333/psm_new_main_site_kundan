...
/* मालमत्ता कर आकारणी प्रमाणपत्र (Property Tax Assessment Certificate). */
const PropertyAssessmentCertificate = () => {
  const { toast, ToastContainer } = useToast();
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [f, setF] = useState({
    ownerName: '', relation: 'वडील', parent: '', address: '',
    propertyNo: '', wardNo: '', usageType: 'निवासी', assessedTax: '', year: '',
  ...
  const canAdd = can(certModuleKey('property-assessment'), 'add');
  const canPrint = can(certModuleKey('property-assessment'), 'print');
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await certificateService.save({
        cert_type: 'property-assessment',
        cert_name: 'मालमत्ता कर आकारणी प्रमाणपत्र',
    ...
    <div className="p-4 sm:p-6 print:p-0">
      ...
      <div className={isView ? '' : 'grid grid-cols-1 gap-6 lg:grid-cols-2'}>
        ...
        <div className="no-print rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">मालमत्ता कर आकारणी प्रमाणपत्र — तपशील भरा</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className={label}>प्रभाग क्रमांक</label>
              <input type="text" name="wardNo" value={f.wardNo} onChange={on} className={inp} placeholder="प्रभाग क्र." />
            </div>
            <div>
              <label className={label}>वापराचा प्रकार</label>
              <select name="usageType" value={f.usageType} onChange={on} className={inp}>
            ...
            <div>
              <label className={label}>कर आकारणी रक्कम (रु.)</label>
              <input type="number" name="assessedTax" value={f.assessedTax} onChange={on} className={inp} placeholder="रक्कम" />
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              ...
              <MarathiInput name="purpose" value={f.purpose} onChange={on} className={inp} placeholder="उदा. बँक कर्जासाठी" />
            </div>
            <div>
        ...
        <div className={isView ? '' : 'lg:sticky lg:top-4 lg:self-start'}>
          <CertificateLayout
            title="मालमत्ता कर आकारणी प्रमाणपत्र"
            subtitle="(Property Tax Assessment Certificate)"
            outwardNo={f.outwardNo}
            canPrint={canPrint}
          >
            <p className="text-justify">
              प्रमाणित करण्यात येते की, श्री./श्रीमती <strong>{f.ownerName || '________________'}</strong>{' '}
              {f.relation} <strong>{f.parent || '________________'}</strong> यांच्या मालकीची मालमत्ता
              क्रमांक <strong>{f.propertyNo || '________'}</strong>, प्रभाग{' '}
              <strong>{f.wardNo || '____'}</strong>, (<strong>{f.usageType}</strong>) या मालमत्तेची
              कर आकारणी सन <strong>{f.year || '________'}</strong> करिता रु.{' '}
              <strong>{f.assessedTax || '________'}</strong> इतकी ग्रामपंचायत दप्तरी नोंदविलेली आहे.
            </p>
            <div className="mt-4">
              <Row k="मालमत्ता क्रमांक" v={f.propertyNo || '__________'} />
              <Row k="प्रभाग क्रमांक" v={f.wardNo || '__________'} />
              <Row k="वापराचा प्रकार" v={f.usageType} />
              <Row k="कर आकारणी रक्कम" v={`रु. ${f.assessedTax || '________'}`} />
...
export default PropertyAssessmentCertificate;
