...
/* नळ जोडणी परवानगी (Pipe Connection Permission). */
const PipeConnectionCertificate = () => {
  const { toast, ToastContainer } = useToast();
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [f, setF] = useState({
    name: '', relation: 'वडील', parent: '', address: '', propertyNo: '',
    connectionType: 'घरगुती', purpose: '',
  ...
  const canAdd = can(certModuleKey('pipe-connection'), 'add');
  const canPrint = can(certModuleKey('pipe-connection'), 'print');
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await certificateService.save({
        cert_type: 'pipe-connection',
        cert_name: 'नळ जोडणी परवानगी',
    ...
    <div className="p-4 sm:p-6 print:p-0">
      ...
      <div className={isView ? '' : 'grid grid-cols-1 gap-6 lg:grid-cols-2'}>
        ...
        <div className="no-print rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">नळ जोडणी परवानगी — तपशील भरा</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className={label}>जोडणी प्रकार</label>
              <select name="connectionType" value={f.connectionType} onChange={on} className={inp}>
                <option value="घरगुती">घरगुती</option>
                <option value="व्यावसायिक">व्यावसायिक</option>
              </select>
            </div>
            <div className="sm:col-span-2">
              <label className={label}>कारण / प्रयोजन</label>
              <MarathiInput name="purpose" value={f.purpose} onChange={on} className={inp} placeholder="उदा. नळ जोडणी मिळणे" />
            </div>
            <div>
        ...
        <div className={isView ? '' : 'lg:sticky lg:top-4 lg:self-start'}>
          <CertificateLayout
            title="नळ जोडणी परवानगी"
            subtitle="(Pipe Connection Permission)"
          ...
          >
            <p className="text-justify">
              ...
              <strong>{f.address || '____________'}</strong>, यांना मालमत्ता क्रमांक{' '}
              <strong>{f.propertyNo || '________'}</strong> करिता{' '}
              <strong>{f.connectionType || '________'}</strong> नळ जोडणी घेण्यास या ग्रामपंचायतीची
              परवानगी देण्यात येत आहे. सदर परवानगी पाणीपुरवठा नियमांच्या अधीन आहे.
            </p>
            <div className="mt-4">
              <Row k="मालमत्ता क्रमांक" v={f.propertyNo || '__________'} />
              <Row k="जोडणी प्रकार" v={f.connectionType || '__________'} />
...
export default PipeConnectionCertificate;
