...
/* थकबाकी नसल्याचा दाखला (No Dues Certificate) — certifies no tax dues pending. */
const NoDuesCertificate = () => {
  ...
  const canAdd = can(certModuleKey('no-dues'), 'add');
  const canPrint = can(certModuleKey('no-dues'), 'print');
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await certificateService.save({
        cert_type: 'no-dues',
        cert_name: 'थकबाकी नसल्याचा दाखला',
    ...
    <div className="p-4 sm:p-6 print:p-0">
      ...
      <div className={isView ? '' : 'grid grid-cols-1 gap-6 lg:grid-cols-2'}>
        ...
        <div className="no-print rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">थकबाकी नसल्याचा दाखला — तपशील भरा</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input type="text" name="upToYear" value={f.upToYear} onChange={on} className={inp} placeholder="उदा. 2025-26" />
            </div>
            <div className="sm:col-span-2">
              <label className={label}>कारण / प्रयोजन</label>
              <MarathiInput name="purpose" value={f.purpose} onChange={on} className={inp} placeholder="उदा. कर्ज / नोंदणी" />
            </div>
            <div>
        ...
        <div className={isView ? '' : 'lg:sticky lg:top-4 lg:self-start'}>
          ...
          >
            <p className="text-justify">
              ...
              <strong>{f.propertyNo || '________'}</strong>, यांचेकडे या ग्रामपंचायतीची कोणतीही
              कर/थकबाकी <strong>{f.upToYear || '________'}</strong> अखेर शिल्लक नाही.
            </p>
            <div className="mt-4">
