...
/* ग्रामपंचायत थकबाकी दाखला (GP Tax Pending / Dues Certificate). */
const GPPendingCertificate = () => {
  const { toast, ToastContainer } = useToast();
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [f, setF] = useState({
    name: '', relation: 'वडील', parent: '', address: '',
    propertyNo: '', pendingAmount: '', upToYear: '',
  ...
  const canAdd = can(certModuleKey('gp-pending'), 'add');
  const canPrint = can(certModuleKey('gp-pending'), 'print');
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await certificateService.save({
        cert_type: 'gp-pending',
        cert_name: 'ग्रामपंचायत थकबाकी दाखला',
    ...
    <div className="p-4 sm:p-6 print:p-0">
      ...
      <div className={isView ? '' : 'grid grid-cols-1 gap-6 lg:grid-cols-2'}>
        ...
        <div className="no-print rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">ग्रामपंचायत थकबाकी दाखला — तपशील भरा</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className={label}>थकबाकी रक्कम (रु.)</label>
              <input type="number" name="pendingAmount" value={f.pendingAmount} onChange={on} className={inp} placeholder="थकबाकी रक्कम" />
            </div>
            <div>
              <label className={label}>वर्षापर्यंत</label>
              <input type="text" name="upToYear" value={f.upToYear} onChange={on} className={inp} placeholder="उदा. सन २०२५-२६" />
            </div>
            <div className="sm:col-span-2">
              <label className={label}>कारण / प्रयोजन</label>
              <MarathiInput name="purpose" value={f.purpose} onChange={on} className={inp} placeholder="उदा. कार्यालयीन कामकाजासाठी" />
            </div>
            <div>
        ...
        <div className={isView ? '' : 'lg:sticky lg:top-4 lg:self-start'}>
          <CertificateLayout
            title="ग्रामपंचायत थकबाकी दाखला"
            subtitle="(GP Tax Dues / Pending Certificate)"
          ...
          >
            <p className="text-justify">
              ...
              <strong>{f.address || '____________'}</strong>, मालमत्ता क्रमांक{' '}
              <strong>{f.propertyNo || '________'}</strong>, यांचेकडे या ग्रामपंचायतीची{' '}
              <strong>{f.upToYear || '________'}</strong> अखेर रु.{' '}
              <strong>{f.pendingAmount || '________'}</strong> इतकी कर थकबाकी येणे बाकी आहे.
            </p>
            <div className="mt-4">
              <Row k="मालमत्ता क्रमांक" v={f.propertyNo || '__________'} />
              <Row k="थकबाकी रक्कम" v={`रु. ${f.pendingAmount || '________'}`} />
              <Row k="वर्षापर्यंत" v={f.upToYear || '__________'} />
...
export default GPPendingCertificate;
