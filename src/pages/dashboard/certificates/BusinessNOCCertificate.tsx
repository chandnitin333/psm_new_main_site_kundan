...
/* व्यवसाय/नोकरीसाठी ना हरकत (Business / Employment NOC). */
const BusinessNOCCertificate = () => {
  const { toast, ToastContainer } = useToast();
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [verifyToken, setVerifyToken] = useState('');
  const [f, setF] = useState({
    name: '', relation: 'वडील', parent: '', address: '', businessName: '',
    businessType: '', location: '', purpose: '',
  ...
  const canAdd = can(certModuleKey('business-noc'), 'add');
  const canPrint = can(certModuleKey('business-noc'), 'print');
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await certificateService.save({
        cert_type: 'business-noc',
        cert_name: 'व्यवसाय/नोकरीसाठी ना हरकत',
    ...
    <div className="p-4 sm:p-6 print:p-0">
      ...
      <div className={isView ? '' : 'grid grid-cols-1 gap-6 lg:grid-cols-2'}>
        ...
        <div className="no-print rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-4 text-lg font-bold text-gray-900 dark:text-white">व्यवसाय/नोकरीसाठी ना हरकत — तपशील भरा</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className={label}>व्यवसायाचे नाव</label>
              <MarathiInput name="businessName" value={f.businessName} onChange={on} className={inp} placeholder="व्यवसायाचे नाव" />
            </div>
            <div>
              <label className={label}>व्यवसायाचा प्रकार</label>
              <MarathiInput name="businessType" value={f.businessType} onChange={on} className={inp} placeholder="उदा. किराणा दुकान" />
            </div>
            <div className="sm:col-span-2">
              <label className={label}>व्यवसायाचे ठिकाण</label>
              <MarathiInput name="location" value={f.location} onChange={on} className={inp} placeholder="ठिकाण" />
            </div>
            <div className="sm:col-span-2">
              <label className={label}>कारण / प्रयोजन</label>
              <MarathiInput name="purpose" value={f.purpose} onChange={on} className={inp} placeholder="उदा. व्यवसाय परवाना मिळणे" />
            </div>
            <div>
        ...
        <div className={isView ? '' : 'lg:sticky lg:top-4 lg:self-start'}>
          <CertificateLayout
            title="व्यवसाय/नोकरीसाठी ना हरकत"
            subtitle="(Business / Employment NOC)"
          ...
          >
            <p className="text-justify">
              ...
              <strong>{f.address || '____________'}</strong>, यांना{' '}
              <strong>{f.location || '________'}</strong> येथे{' '}
              <strong>{f.businessName || '________'}</strong> (<strong>{f.businessType || '________'}</strong>)
              हा व्यवसाय करण्यास/चालविण्यास या ग्रामपंचायतीची कोणतीही हरकत नाही.
            </p>
            <div className="mt-4">
              <Row k="व्यवसायाचे नाव" v={f.businessName || '__________'} />
              <Row k="व्यवसायाचा प्रकार" v={f.businessType || '__________'} />
              <Row k="व्यवसायाचे ठिकाण" v={f.location || '__________'} />
...
export default BusinessNOCCertificate;
